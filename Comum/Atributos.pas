...
type
  //Pedro
  TStatusTelaLancamentoCaixa = (stInserindoCaixa, stEditandoCaixa, stExcluindoCaixa);
  TTipoMovimentacao = (tmEntrada,tmSaida);
  TSituacaoCreditoCliente = (scCliBloqueado,scClienteLiberado);
  TTipoOrcamento = (toOrcamento,toPreVenda,toFaturado,toDevolucao);
  TTipoAvisoEstoque = (taEstoqueNaoAvisar,taEstoqueAvisar,taEstoqueBloquear);
  TStatusTelaAtributos = (staNavegandoGrid, staInserindo, staEditando, staNavegandoEdits);
  TTipoProduto = (tpProduto, tpServico);
  TTipoSegmentoEmpresa = (tseGeral,tseMatConstrucao,tseMadeireira,tseConstrutora,tseMercados,tseRestaurante,tseServicosGeral,tseServicoMecanica);

  //pedro usado no TFPessoa
  TTipoPessoa = (tpCliente,tpFornecedor,tpColaborador,tpConvenio,tpContador,tpTransportadora);

  //Pedro usado Ordem Serviço
  TTipoFaseOS = (tfosDigitacao,tfosAnalise,tfosOrcamento,tfosExecucao,tfosFaturamento,tfosConcluido,tfosDevolvido);

  TTipoLancamentoCaixa = (tpReceita, tpDespesa, tpTransferir, tpEstornar);

  TLocalDisplay = (ldGrid, ldLookup, ldComboBox, ldNaoVisible);
  ...
  private
    ...
    FCampoConsultaTransiente : string;
  public
    constructor Create(pName: String); overload;
    ...
    constructor Create(pName: String; pCaption: String; pLength: Integer; pLocalDisplay: TLocalDisplayColumn; pTransiente: Boolean; pTableName: String; pLocalColumn: String; pForeingColumn: String; pCampoConsultaTransiente: string = ''); overload;
    ...
    // campo para ser utilizado na consulta quando houver um join
    property CampoConsultaTransiente : string read FCampoConsultaTransiente write FCampoConsultaTransiente;
  ...
  // Domain
  //Pedro - 20/06/2014
  TDomain = class(TCustomAttribute)
  private
    FName: String;
  public
...
constructor TColumn.Create(pName: String; pCaption: String; pLength: Integer; pLocalDisplay: TLocalDisplayColumn; pTransiente: Boolean; pTableName: String; pLocalColumn: String; pForeingColumn: String; pCampoConsultaTransiente: string = '');
begin
  Create(pName, pCaption, pLocalDisplay, pTransiente);
  ...
  FCampoConsultaTransiente := pCampoConsultaTransiente;
end;
...
constructor TDomain.Create(pName: String);
begin
  FName := pName;
end;
